...
relay(中继) 不持有 SupervisorHandle(监督器句柄). command forwarding(命令转发) 只能在 authenticated session(已认证会话) 建立, 目标完成 active registration(活动注册), session(会话) 授权范围匹配, 并且目标已经绑定 IPC(进程间通信) 后发生.
